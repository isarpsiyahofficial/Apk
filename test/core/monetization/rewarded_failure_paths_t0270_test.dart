...
void main() {
  ...
  for (final scenario in <(
    RewardedAdTerminalOutcome,
    String,
  )>[
    (
      RewardedAdTerminalOutcome.cancelled,
      RewardedShareUnlockT0269.cancelledLocalizationKey,
    ),
    (
      RewardedAdTerminalOutcome.failed,
      RewardedShareUnlockT0269.failedLocalizationKey,
    ),
    (
      RewardedAdTerminalOutcome.noFill,
      RewardedShareUnlockT0269.noFillLocalizationKey,
    ),
  ]) {
    test('${scenario.$1.name} gives no reward and never forces PRO', () async {
      final flow = await readyFlow();
      flow.begin(
        designId: 'locked-design-0270',
        isDesignLocked: true,
        entitlement: const EntitlementState.free(),
      );

      final result = flow.finish(
        designId: 'locked-design-0270',
        outcome: scenario.$1,
      );
  ...
  test('completed outcome is the only terminal state that grants a use', () async {
    final flow = await readyFlow();
    flow.begin(
      designId: 'locked-design-success',
      isDesignLocked: true,
      entitlement: const EntitlementState.free(),
    );
  ...
  test('terminal result clears active attempt so callbacks cannot double grant', () async {
    ...
    final first = flow.finish(
      designId: 'locked-design-race',
      outcome: RewardedAdTerminalOutcome.noFill,
    );
